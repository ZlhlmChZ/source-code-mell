...
$opts = db_query_fetch_all('SELECT * FROM categories WHERE instanceID = \''.$_SESSION["IID"].'\' ORDER BY title');
...
foreach ($filesDuplicate as $file) {
  echo '
      <tr>
          <td>
              <a href="../download.php?id=',htmlspecialchars($file['id']),'">',htmlspecialchars($file['title']),'</a>
          ...
          <td>';
            form_start(CONFIG_SITE_ADMIN_RELPATH . 'actions/edit_challenge', 'no-padding-or-margin');
            form_hidden('action', 'purge_file');
            form_hidden('id', $file['id']);
            form_hidden('challenge_id', $_GET['id']);
            form_button_submit('Delete (Cloned File)');
            form_end();
          echo '
          </td>
      </tr>
