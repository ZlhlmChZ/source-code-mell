...
$challenges =array_values(db_select_one("challenges",array ('COUNT(*)'),array('category'=>$cat['id'])))[0];
    if ($time < $cat['available_from'] || $time > $cat['available_until']) {
	...
	//echo $challenges;
        echo '<li ',($current_category['id'] == $cat['id'] ? ' class="active"' : ''),'><a href="',CONFIG_SITE_URL,'challenges?category=',htmlspecialchars($cat['id']),'">',htmlspecialchars($cat['title']),'&nbsp','(',$challenges,')','</a></li>';
    }
}
echo '</ul>
</div>';
...
if ($current_category['description']) {
    echo '<div id="category-description">', $current_category['description'], '</div>';
}
        ...
        if (!$challenge['correct_submission_added'] && $time < $challenge['available_until']) {
		    ...
		    if($hint['value'] == 0)
	            ...
	            $hint1 = db_select_one(
                    'purchases',
                    array('*'),
                    array('hid' => $hint['id'], 'uid' => $_SESSION['id'])
                    );
               ...
               if (cache_start('files_' . $challenge['id'], CONFIG_CACHE_TIME_FILES)) {
                    $files = db_select_all(
                     ...
                     $filesDuplicate = db_select_all(
                        ...
                        array('challenge' => $challenge['cloneOf'])
                    );
